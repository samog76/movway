...
node scripts/release-manifest.mjs   # version.json — MUST run, or no device is offered the update
...
## 8b. How the in-app updater works

Movway is sideloaded, so nothing updates it. `Settings → Updates` checks for a newer build and
installs it on the device.

- `download-site/version.json` is the manifest, generated by `scripts/release-manifest.mjs`.
  It refuses to run if `package.json` and `build.gradle` disagree, because a manifest claiming
  a version the APK does not carry puts every device in a loop: install, still report the old
  version, be offered the same update again.
- The default source is the repo itself — `raw.githubusercontent.com/samog76/movway/main/
  download-site/version.json`. It is public, it is already where shipping pushes the APK, and
  it needs no deploy of its own to be current. Settings can point elsewhere.
- `UpdatePlugin.java` does what the WebView cannot: fetches over the platform's HTTP stack,
  checks the SHA-256 before anything is installed, and hands the file to Android's package
  installer through the FileProvider at `${applicationId}.fileprovider`. **That authority must
  match the one in AndroidManifest.xml or the install crashes** — verify with
  `aapt2 dump xmltree --file AndroidManifest.xml download-site/movway.apk`.
- It downloads to `getCacheDir()`, which the existing `@xml/file_paths` already shares via
  `cache-path`. Moving it to `getExternalFilesDir()` would need a new `external-files-path`
  entry or `getUriForFile` throws.
- Local plugins are not auto-discovered: `MainActivity.onCreate` calls
  `registerPlugin(UpdatePlugin.class)` before `super`. Without it the web layer's calls
  silently resolve to nothing.
- Version comparison is numeric per segment, not string order — "1.10.0" must beat "1.9.0".
  Version *code* wins when both sides have one, since that is what Android orders installs by.

The updater cannot deliver itself: a device must be on 1.9.0 or later before any of this runs,
so that one install is still by hand.
